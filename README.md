...
- **实时统计面板**：
  ...
  - Top 10 高频按键排行；
  - 最近按键流水条。
- **控制中心窗口（tkinter）**：启动即显示，实时显示运行状态（累计/今日/速率/模式/地址/数据路径），按钮一键「打开可视化页面 / 打开数据目录 / 退出程序」；点窗口 X 最小化到托盘（不退出）。内置**设置区**（右下角通知开关、每日统计通知开关）与**数据区**（导出/导入统计数据，支持合并或覆盖）。
- **系统托盘常驻（零依赖实现）**：托盘右键菜单「显示控制窗口 / 打开可视化页面 / **暂停采集** / 打开数据目录 / 退出」；双击图标弹出控制窗口，气泡提示实时显示累计按键数。
- **右下角通知推送**：启动 / 停止 / 暂停恢复时弹出系统通知，可在控制窗口设置中开关。
- **多种网页显示模式**：顶部中央一键切换 —— **经典柱**（细柱立在键帽上方）/ **覆盖柱**（柱体铺满整个按键，从键帽表面完全盖住，如空格键被整根长柱覆盖）/ **热力图**（纯热力着色，无柱体）；切换带平滑过渡动画，选择会被记住。
...
不想装 Python 环境？可直接打包成**单个可执行文件**（Windows）：
...
# 运行打包产物（无黑窗口；演示模式：--demo）
dist\keyboard-peak.exe
dist\keyboard-peak.exe --demo
```

> 打包版数据自动保存在 `%LOCALAPPDATA%\keyboard-peak\`（不会随程序退出丢失）。
